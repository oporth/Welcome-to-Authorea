...
Near the poles, the wind power steeply declines and the shock retreats back towards the pulsar while it bulges out further in the equatorial plane where the wind power is maximal.  The resulting shape of the oblique shock and radiative signatures of particles emitting close to the shock will be discussed further in (SECTION YY).  

Depending on their local wind magnetization, streamlines that pass over the shock behave qualitatively very different.  
In high-$\sigma$ regions i.e. close to the axis, the flow can remain highly relativistic with downstream Lorentz factors larger than $\sigma^{1/2}$, the value obtained for the perpendicular shock.  For $\sigma\to\infty$, streamlines will simply continue to flow out radially after traversing the shock.  For non-vanishing shock compression on the other hand, the force-free equilibrium of the wind is destroyed and flow-lines start curving towards the pole due to the pinch-force of the magnetic field.  This mechanism of jet formation due to \cite{lyubarsky2002} has been observed in axisymmetric \citep[e.g.][]{komissarov2003a} and full 3D simulations \citep[e.g.][]{PorthKomissarov2014a}.  
In the low-sigma e.g. striped regions, due to the conservation (compression) of the tangential (normal) velocity fields, streamlines experience shock aberration and instead get  deflected towards the equatorial plane.  This leads to the occurrence of ``wisps'' to be further discussed in (SECTION XX).  
...
An overview of the large-scale dynamics in the 3D models is given in Figure \ref{fig:slices}.  As the z-pinch is rapidly destroyed, the total pressure is now more or less uniform and resembles the purely hydrodynamic models even for highly magnetised wind.  Only at the base of the plumes flow compression is significantly enhanced.  
The magnetic field strength exhibits a significant range with maximal values in the ``arch flow '' just on top/below the oblique TS.  As long as axisymmetry is approximately conserved, the magnetic field strength follows the scalings discussed in section \ref{sec:nebula}, hence increases until the magnetic energy density reaches equipartition with the plasma pressure.  Throughout the torus region, magnetic field remains strong, however fluid instabilities in the fast equatorial shear flow lead to turbulence starting at $\approx 2 $ shock radii.  It is interesting to note that the oblique shock compresses streamlines that carry magnetic field with opposing polarity towards the equator, turning the null-line into a large-scale current sheet.  

A  closeup view of the TS is shown in Figure \ref{fig:slices-sigma}.  
