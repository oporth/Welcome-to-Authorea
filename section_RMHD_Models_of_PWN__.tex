...
In this vein, in figure \ref{fig:maps}, we show synthetic emission maps of the region around the TS from two 3D simulations: one with parameters $\sigma_0=3,\alpha=10^\circ$ (left) and one with $\sigma_0=1,\alpha=45^\circ$ (right).  While the right-hand panel provides a good match to the morphology in Crab, the torus structure is suppressed and emission from the polar jet dominates in the left-hand panel.  This result is not surprising: as the extent of the striped zone is decreased, more streamlines are re-focussed into the jet (cf. figure 14 of \cite{PorthKomissarov2014a}).  The recent 3D simulations presented by \cite{Olmi2016} [HOW CAN I CITE YOU?] confirm this finding.  Hence the jet/torus flux ratio could yield a valuable handle on the pulsar obliqueness.  
...
Moving away from the TS, the uncertainties in radiative models increase:  Currently, the radiative models tend to over-predict the contrast between torus and ambient emission, as well as the radio polarization degree.
